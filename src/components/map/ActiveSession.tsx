...
type SessionPhase = 'navigating' | 'arriving' | 'unlocking' | 'active' | 'ending';

export function ActiveSession({ space, hours, onEndSession, transactionId }: ActiveSessionProps) {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        const response = await fetch(`/api/space/${space.id}/status`).catch(() => null);
        if (response?.ok) {
        ...
        <div className="flex justify-center">
          {phase === 'navigating' && (
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">The gate will unlock when you arrive</p>
              </div>
          ...
          {phase === 'unlocking' && (
            <div className="text-center space-y-4">
              <div className="w-24 h-24 mx-auto bg-orange-100 rounded-full flex items-center justify-center">
                <svg className="w-12 h-12 text-orange-600 animate-spin" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 11V7a4 4 0 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z" />
                </svg>
              </div>
              <div>
                <p className="font-medium text-orange-600">Unlocking Gate...</p>
                <p className="text-sm text-gray-500">Sending signal to bollard</p>
              </div>
            </div>
          )}
              ...
              <div className="bg-gray-100 rounded-lg p-4">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-mono font-bold text-lg">
                    {Math.floor(remainingMinutes / 60)}:{(remainingMinutes % 60).toString().padStart(2, '0')}
                  </span>
                </div>
                <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-green-500 rounded-full transition-all duration-1000"
                    style={{ width: `${(remainingMinutes / (hours * 60)) * 100}%` }}
                  />
                </div>
              </div>
          ...
          {phase === 'ending' && (
            <div className="text-center space-y-4">
              ...
              <div>
                <p className="font-medium text-purple-600">Processing Payment...</p>
                <p className="text-sm text-gray-500">Sending ${totalCost.toFixed(2)} via Visa Direct</p>
              </div>
            </div>
          )}
        </div>
