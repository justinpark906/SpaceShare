...
const CAPITAL_ONE_CONFIG = {
  baseUrl: process.env.CAPITAL_ONE_BASE_URL || 'https://api-sandbox.capitalone.com',
...
export async function POST(request: NextRequest) {
  try {
    ...
    const transferPayload = {
      ...
      memo: 'EcoSquare Automatic Tax Sweep',
      metadata: {
        source: 'ecosquare',
        transactionId,
        type: 'tax_savings',
    ...
    console.log('📤 Capital One Transfer Payload:', JSON.stringify(transferPayload, null, 2));
    ...
    return NextResponse.json({
      ...
      breakdown: {
        originalPayout: amount / 0.20, // Calculate original payout
        taxSweepRate: '20%',
        amountSwept: amount,
        remainingPayout: (amount / 0.20) - amount,
      },
    });
  } catch (error) {
...
export async function GET(request: NextRequest) {
  ...
  const savingsData = {
    ...
    totalSwept: 156.80, // Total swept this month
    taxVaultBalance: 892.40, // Total in tax savings
    sweepHistory: [
