...
const CONFIG = {
  ...
  trackerName: process.env.TRACKER_NAME || 'EcoSquareTracker',
...
async function updateDevicePosition(longitude: number, latitude: number): Promise<void> {
  console.log(`📍 Updating position: (${latitude.toFixed(6)}, ${longitude.toFixed(6)})`);
...
async function getDevicePosition(): Promise<void> {
  try {
    const result = await client.send(new GetDevicePositionCommand({
      TrackerName: CONFIG.trackerName,
      DeviceId: CONFIG.deviceId,
    }));
...
async function simulateMarcusArrival(): Promise<void> {
  ...
  console.log(`🎯 Destination: Sarah's Parking Spot (${SARAH_PARKING_SPOT.latitude}, ${SARAH_PARKING_SPOT.longitude})\n`);
  ...
  console.log('Step 2: Marcus ARRIVES at the parking spot (entering geofence!)');
  await updateDevicePosition(SARAH_PARKING_SPOT.longitude, SARAH_PARKING_SPOT.latitude);
...
async function main() {
  ...
  console.log('╔═══════════════════════════════════════════╗');
  console.log('║     EcoSquare Location Simulator          ║');
  console.log('╚═══════════════════════════════════════════╝\n');
  ...
  switch (command) {
    ...
    default:
      console.log('Usage:');
      console.log('  npx tsx scripts/mock_location_update.ts arrive  - Simulate Marcus arriving');
      console.log('  npx tsx scripts/mock_location_update.ts leave   - Simulate Marcus leaving');
      console.log('  npx tsx scripts/mock_location_update.ts status  - Get current position');
  }
}
